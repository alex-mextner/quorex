...
- Run profile precedence: `CLI direct flags > CLI --run-profile > merged config run_profile > local config > global config > embedded defaults`
...
- `run_profile` config option: name of the active run profile. Selects a `[run_profile.<name>]` section that bundles `claude_command`, `claude_args`, `task_model`, `review_model`, `external_reviewers`, `external_review_tool`, `custom_review_script` as one unit. CLI flag `--run-profile` takes precedence, including `--run-profile=` to disable a configured profile for one run. Profile is applied after config files but before direct CLI flags, so `--run-profile=x --claude-command=foo` applies profile x then overrides only `claude_command`. Returns a clear error when the named profile is not defined or `external_review_tool` is invalid. Parsed in `pkg/config/config.go:ApplyProfile`. Fields missing from a profile section fall back to main config; same-name profile sections merge per field across config levels; fields present with empty values clear lower-precedence profile values. If a profile sets both `external_reviewers` and `external_review_tool`, named reviewers win. Disabled by default (empty)
- `task_model` config option: model[:effort] for task execution (e.g., `opus`, `opus:high`, `:medium`). Effort values: `low`, `medium`, `high`, `xhigh`, `max`. CLI flag `--task-model` takes precedence, including `--task-model=` to clear a configured value for one run. Parsed in `ParseModelEffort` (pkg/processor/runner.go), split on first colon. Appended to `claude_command` as `--model <m>` and/or `--effort <e>`; custom wrappers may ignore or implement the flags. Disabled by default (empty = Claude CLI defaults)
- `review_model` config option: model[:effort] for review phases. Falls back to `task_model` if empty. CLI flag `--review-model` takes precedence, including `--review-model=` to clear a configured value for one run. Same wrapper behavior and syntax as `task_model`. Disabled by default
...
- `external_reviewers` config option: comma-separated named reviewers to run in parallel. Reviewer definitions use `[external_reviewer.<name>]` sections with `driver` (codex|script|none) and optional `script` fields. When set, takes precedence over `external_review_tool`. CLI flag `--external-reviewers` overrides for one run; empty value disables external review. Conflicts with `--external-review-tool`. A single `codex` reviewer uses the Codex evaluation prompt; multiple parallel reviewers are evaluated together with `custom_eval.txt`
